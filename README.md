...
Prairie Push Relay lets a self-hosted Prairie Server wake the Prairie app when something
needs its attention. It provides the small piece of shared infrastructure needed
to reach platform push services without putting Prairie's provider credentials on
...
user names, profile names, and server address stay between your Prairie Server and
your devices.

## How it works

1. A Prairie Server registers with the relay and receives an opaque deployment ID
...
Delivery requests are idempotent. This allows a Prairie Server to retry safely
...
network close to the sending Prairie Server, and the service can scale without a
...
or long-running database for the Prairie team to expose or maintain.
...
- the IP address the self-hosted Prairie Server uses to contact the relay, for
...
The relay does not have Prairie user accounts, does not know who a device belongs
